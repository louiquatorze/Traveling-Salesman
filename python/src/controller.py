...
class Controller(QObject):
    ...
    def random(self):
        ...
        self.graph.setPath(np.empty((0, 2), dtype=np.float64))
    ...
    def renderPaths(self, paths, min_path):
        if min_path < 0:
            self.graph.setPath(paths[0])
        else:
            self.graph.setMinPath(paths[min_path])
    ...
    def createSolverThread(self, indent = 0):
        ...
        self.solver_worker = SolverWorker(self.memory, init_data, self.worker_paused, self.calcDelay())
